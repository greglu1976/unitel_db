...
# СОЗДАЕМ ОТЧЕТ
    ...
    # ищем шкаф в базе
    cabinet = Cabinets.objects.get(pk=cab)
    return
'''
    if cabinet.terminal1: # если есть терминал 1 то добавляем таблицу
        ...
        connections = PhDLDconnections.objects.all().filter(IEDname=cabinet.terminal1)

        for item in connections:
            print('---->', item.LDname)
            ld_short_eng_name_field = str(item.LDname)  # взяли имя для второго столбца ATCC/....
